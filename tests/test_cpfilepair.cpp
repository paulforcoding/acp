...
TEST_CASE("CPFilePair symlink copy", "[cpfilepair]")
{
    ...
    CPFilePair p(src_link, dst_dir + "/link.txt", 4096, false, false, false, logger);
...
TEST_CASE("CPFilePair dangling symlink", "[cpfilepair]")
{
    ...
    auto logger = std::make_shared<ConsoleLogger>();
    CPFilePair p(src_link, dst_dir + "/dangling_link", 4096, false, false, false, logger);
    auto init_res = p.CheckAndInit();
    REQUIRE(init_res.has_value());
    REQUIRE(p.IsSymlink());
...
TEST_CASE("CPFilePair symlink overwrites existing dst", "[cpfilepair]")
{
    ...
    auto logger = std::make_shared<ConsoleLogger>();
    CPFilePair p(src_link, dst_link, 4096, false, false, false, logger);
    auto init_res = p.CheckAndInit();
    REQUIRE(init_res.has_value());
...
TEST_CASE("CPFilePair directory copy", "[cpfilepair]")
{
    ...
    auto logger = std::make_shared<ConsoleLogger>();
    CPFilePair p(src_dir, dst_dir, 4096, false, false, false, logger);
    auto init_res = p.CheckAndInit();
    REQUIRE(init_res.has_value());
    REQUIRE(p.IsDir());
...
TEST_CASE("CPFilePair truncate and fsync", "[cpfilepair]")
{
    ...
    auto logger = std::make_shared<ConsoleLogger>();
    CPFilePair p(src, dst, 4096, false, true, false, logger);
    auto init_res = p.CheckAndInit();
    REQUIRE(init_res.has_value());
