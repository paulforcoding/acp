...
TEST_CASE("CPFilePair CheckAndInit skips socket file", "[integration][cpfilepair]")
{
    ...
    CPFilePair p(src_sock, dst_sock, opts.IoSize, opts.DirectIO, opts.SyncWrites, /*cksum*/ false, false, logger, nullptr);
...
TEST_CASE("CPFilePair CheckAndInit with real file", "[integration][cpfilepair]")
{
    ...
    CPFilePair p(src, dst, opts.IoSize, opts.DirectIO, opts.SyncWrites, /*cksum*/ false, false, logger, nullptr);
    auto init_res = p.CheckAndInit();
