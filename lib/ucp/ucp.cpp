...
tl::expected<void, StackError> UIOSlotMgr::Init()
{
    ...
    if (ret < 0)
    {
        return tl::unexpected(StackError(fmt::format("io_uring_queue_init() failed: {}", -ret)));
    }
...
tl::expected<void, StackError> UIOSlotMgr::SubmitOneRead(UIOSlot *slot)
{
    ...
    if (ret < 0)
    {
        return tl::unexpected(StackError(fmt::format("io_uring submit read failed: {}", -ret)));
    }
...
tl::expected<int, StackError> UIOSlotMgr::SubmitReads()
{
    int submitted = 0;
...
tl::expected<void, StackError> UIOSlotMgr::SubmitOneWrite(UIOSlot *slot)
{
    ...
    if (ret < 0)
    {
        return tl::unexpected(StackError(fmt::format("io_uring submit write failed: {}", -ret)));
    }
...
tl::expected<int, StackError> UIOSlotMgr::SubmitWrites()
{
    int submitted = 0;
    for (auto slot : m_slots)
    {
...
tl::expected<void, StackError> UIOSlotMgr::ReapRead(struct io_uring_cqe *cqe)
{
    ssize_t res = cqe->res;
    ...
    if (res < 0)
    {
        ...
        return tl::unexpected(StackError(fmt::format("Uring read failed: {}", -res)));
    }
    if (res == 0)
    {
        return tl::unexpected(StackError("Uring read returned 0 bytes, unexpected."));
    }
    ...
    if (!submit_res)
    {
        if (submit_res.error() == StackError("EAGAIN"))
        {
            m_logger->debug("SubmitOneWrite() in ReapRead() got EAGAIN after read reap, slot: {}, will try later.", slot->GetID());
            PrtSlots();
            return {};
        }
        return tl::unexpected(StackError("SubmitOneWrite() failed after read reap, err: ", submit_res.error()));
    }
    return {};
}

tl::expected<void, StackError> UIOSlotMgr::ReapWrite(struct io_uring_cqe *cqe)
{
    ssize_t res = cqe->res;
    UIOSlot *slot = static_cast<UIOSlot *>(io_uring_cqe_get_data(cqe));
    assert(slot != nullptr);
    if (res < 0)
    {
        ...
        return tl::unexpected(StackError(fmt::format("Uring write failed: {}", -res)));
    }
    if (res == 0)
    {
        return tl::unexpected(StackError("Uring write returned 0 bytes, unexpected."));
    }
...
tl::expected<void, StackError> UIOSlotMgr::IOReap()
{
    ...
    if (ret < 0)
    {
        return tl::unexpected(StackError(fmt::format("io_uring_submit_and_wait failed: {}", -ret)));
    }
...
tl::expected<void, StackError> UIOSlotMgr::RunQueue(CPFilePair *file_infos)
{
...
tl::expected<void, StackError> UIOSlotMgr::CheckStuck()
{
    bool isStuck = true;
    for (auto slot : m_slots)
    {
    ...
    if (isStuck)
    {
        ...
        return tl::unexpected(StackError("Detected stuck uring operations."));
    }
...
tl::expected<void, StackError> UFileCopy::Copy(const char *src_path, const char *dst_path)
{
    auto fis{std::make_unique<CPFilePair>(src_path, dst_path)};
    auto init_res = fis->CheckAndInit();
    if (!init_res)
    {
        return tl::unexpected(StackError("CPFilePair::CheckAndInit(), err: ", init_res.error()));
    }

    auto run_res = m_io_slot_mgr.RunQueue(fis.get());
    if (!run_res)
    {
        return tl::unexpected(StackError("m_io_slot_mgr.RunQueue(), err: ", run_res.error()));
    }

    auto truncate_res = fis->TrucateDstToSrcSize();
    if (!truncate_res)
    {
        return tl::unexpected(StackError("CPFilePair::TrucateDstToSrcSize(), err: ", truncate_res.error()));
    }
