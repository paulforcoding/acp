...
tl::expected<void, zplib::StackError> CPFilePair::CheckAndInit()
{
    ...
    {
        return tl::unexpected(zplib::StackError("Source file is not a regular file: " + m_src_path));
    }
...
tl::expected<std::shared_ptr<CPFilePair>, zplib::StackError> CPFilePairMgr::GetNextReadIO()
{
...
AGAIN:
    ...
    if (!(*mReadPtr)->IsInitialized())
    {
        ...
        if (!init_res)
        {
            ...
            return tl::unexpected(zplib::StackError("(*mReadPtr)->CheckAndInit(), err: ", init_res.error()));
        }
    }
    ...
    if ((*mReadPtr)->IsDir())
    {
        ...
        m_logger->debug("Skipping copy directory file pair from mFilePairs, src: {}, dst: {}",
    ...
    if ((*mReadPtr)->IsReadFinished())
    {
        ...
        m_logger->debug("Finished read file pair from mFilePairs, src: {}, dst: {}, remaining pairs: {}",
...
tl::expected<void, zplib::StackError> CPFilePairMgr::CheckWriteComplete(std::shared_ptr<CPFilePair> writeIt)
{
    assert(writeIt != nullptr);
    m_logger->debug("Checking write completion for file pair: src: {}, dst: {}",
                    writeIt->GetSrcPath(), writeIt->GetDstPath());

    if (writeIt->IsWriteFinished())
    {
        auto truncate_res = writeIt->TrucateDstToSrcSize();
        if (!truncate_res)
        {
            return tl::unexpected(zplib::StackError("writeIt->TrucateDstToSrcSize(), err: ", truncate_res.error()));
        }

        // 如果有其他结尾要做的事情，比如copy file attributes，可以在这里做

        m_logger->debug("Completed copying file pair: src: {}, dst: {}, size: {}",
                        writeIt->GetSrcPath(), writeIt->GetDstPath(), writeIt->GetSrcFileSize());
        // 从inflight列表中移除

        mInflightFPs.remove(writeIt);
